...
void getMotifGraph(vector<MotifSignalFrame>* mt){
  ...
  ofstream outputFile("tmp/test_mp_result/fpsig", ios_base::trunc );
  ofstream consFile("tmp/test_mp_result/consSig", ios_base::trunc );
  ofstream motifFile("tmp/test_mp_result/motifOrder.txt", ios_base::trunc );
...
void fpMatch(vector<centroid> &C, vector<MotifSignalFrame>* f, bool getCons ){
  ...
  struct stat st = {0};
  //create directory if it doesn't exists
  if (stat("tmp/test_mp_non_cons", &st) == -1) {
    mkdir("tmp/test_mp_non_cons", 0700);
  }
  ofstream outputFile("tmp/test_mp_non_cons/testing_flip_fpsig", ios_base::trunc );
  ofstream consFile("tmp/test_mp_non_cons/testing_flip_consSig", ios_base::trunc );
  ofstream tempFile("tmp/test_mp_non_cons/testing_filp_temp", ios_base::trunc );
  ofstream anticFile("tmp/test_mp_non_cons/anticorrelation_lv.txt", ios_base::trunc );
  ofstream countFile("tmp/test_mp_non_cons/count.txt", ios_base::trunc );
  ofstream CIFile("tmp/test_mp_non_cons/cons_CI.txt", ios_base::trunc );
  for(int i=0; i<temp_C.size(); i++){
    outputFile <<"\"[DEBUG]index " << i <<" centroid... \"\n";
